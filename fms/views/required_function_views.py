...
def required_function_entry(request):
    try:
        ...
        rf = FunctionMaster.objects.get(function_cd=required_function)
        ...
        # 更新時の処理
            ...
            # 作業中の要求機能のレコードがある場合の処理
                ...
                budget_required_function_data = BudgetRequiredFunction.objects.get(budget_id=budget_id, required_function=rf, sub_no=this_sub_no)
                # 各項目の値を格納
...
@login_required
@require_POST
def required_function_data_info(request):
    try:
        # JSからのPOST引数を取得・・・数値項目は数値に変換(int関数)
        ...
        if edit_flag == 1:
            ...
            return render(request, 'fms/parts/budget/required_function/required_function_info.html', data)
    except Exception:
        output_log_exception(request, traceback.format_exc())
        raise
...
@login_required
@require_POST
def required_function_data_info_2(request):
    try:
        # JSからのPOST引数を取得・・・数値項目は数値に変換(int関数)
        target_id = int(request.POST["budget_id"])
        budget_required_function_id = int(request.POST["budget_required_function_id"])
...
# 登録済の要求機能の表示処理
@require_POST
def required_function_list(request):
    try:
        # JSからのPOST引数を取得・・・数値項目は数値に変換(int関数)
        ...
        return render(request, 'fms/parts/budget/required_function/required_function_lists.html', data)
    except Exception:
        output_log_exception(request, traceback.format_exc())
        raise
