...
@login_required
@require_POST
def execution_proinspectionresults_data_info(request):
    try:
        ...
        # 対象のデータがあるかチェック
        proinspectioneesults_data_num = ProInspectionResults.objects.filter(budget_id=budget_id, construction_id=construction_id, lost_flag=0).count()
        # ある場合各項目値取得
        if proinspectioneesults_data_num > 0:
            proinspectioneesults_data = ProInspectionResults.objects.get(budget_id=budget_id, construction_id=construction_id, lost_flag=0)

            rev_no = proinspectioneesults_data.rev_no if proinspectioneesults_data.rev_no is not None else ""
            budget_no = base_data['budget_no']
            budget_name = base_data['budget_name']
            sub_id = proinspectioneesults_data.sub_id if proinspectioneesults_data.sub_id is not None else ""

            order_classification_from = proinspectioneesults_data.order_classification_from if proinspectioneesults_data.order_classification_from is not None else ""
            order_classification_to = proinspectioneesults_data.order_classification_to if proinspectioneesults_data.order_classification_to is not None else ""
            inspection_results = proinspectioneesults_data.inspection_results if proinspectioneesults_data.inspection_results is not None else ""
            inspection_date = proinspectioneesults_data.inspection_date if proinspectioneesults_data.inspection_date is not None else ""
            judge_kbn = proinspectioneesults_data.judge_kbn if proinspectioneesults_data.judge_kbn is not None else ""
        # ない場合空値
        else:
            proinspectioneesults_data = ""
        ...
        old_proinspectioneesults_data_num = ProInspectionResults.objects.filter(budget_id=budget_id, construction_id=construction_id, lost_flag=1).count()
        # 無効となった(=1つ前のrev_noの)対象のデータのレコードがある場合
        if old_proinspectioneesults_data_num > 0:
            # 無効となった(=1つ前のrev_noの)対象のデータを取得
            old_proinspectioneesults_data = ProInspectionResults.objects.filter(budget_id=budget_id, construction_id=construction_id, lost_flag=1).all().order_by('-id')[0]
        else:
            old_proinspectioneesults_data = ""

        # データ編集機能要否判定
        work_edit_action_num = 0
        # 対象stepで「ProInspectionResults」がデータ更新対象か確認
        work_edit_action_num = work_edit_action_num + DataEntryStepMaster.objects.filter(step_id=present_step, target_table='proinspectionresults').count()
        edit_flag = 0
        ...
        if copy_check == 0:
            # タブ毎のボタン表示対応
            stepdisplayitem_data = StepDisplayItem.objects.get(step=present_step, div_id_name='execution_inspectionresults', lost_flag=0)
        ...
        data = {
            ...
            'proinspectioneesults_data_num': proinspectioneesults_data_num,
            'proinspectioneesults_data': proinspectioneesults_data,
            'old_proinspectioneesults_data_num': old_proinspectioneesults_data_num,
            'old_proinspectioneesults_data': old_proinspectioneesults_data,
        ...
        if edit_flag == 1:
            return render(request, 'fms/parts/execution/execution_proinspectionresults/execution_proinspectionresults_edit.html', data)
        else:
            return render(request, 'fms/parts/execution/execution_proinspectionresults/execution_proinspectionresults_info.html', data)
    except Exception:
...
@login_required
@require_POST
def execution_proinspectionresults_entry(request):
    try:
        DIFF_JST_FROM_UTC = 9
        ...
        if request.POST['order_classification_to'] != "":
            order_classification_to_date = request.POST['order_classification_to']
        else:
            order_classification_to_date = request.POST['order_classification_from']
        ...
        if new_record == True:
            # 設定した予算ID, 工事IDで新規作成
            proinspectioneesults_data, created = ProInspectionResults.objects.get_or_create(budget_id=budget_id, construction_id=construction_id, entry_on_progress_flag=0, lost_flag=0)
            # 登録の日時、登録者を登録
            proinspectioneesults_data.entry_datetime = now
            proinspectioneesults_data.entry_operator = operator
            # 工事納入情報テーブルを保存
            proinspectioneesults_data.save()
            # 登録日時、登録者で工事納入情報テーブルを抽出
            proinspectioneesults_data = ProInspectionResults.objects.get(entry_datetime=now, entry_operator=operator, lost_flag=0)
            # 主キーを取得
            proinspectioneesults_unique_id = proinspectioneesults_data.id
            # 主キーで工事納入情報テーブルを抽出
            proinspectioneesults_data = ProInspectionResults.objects.get(id=proinspectioneesults_unique_id, lost_flag=0)
            # rev_no、作業中FL、無効FLに値を代入
            proinspectioneesults_data.rev_no = 0
            proinspectioneesults_data.entry_on_progress_flag = 1
            proinspectioneesults_data.lost_flag = 0
            # 工事納入情報テーブルを保存
            proinspectioneesults_data.save()
        # 更新時の処理
        else:
            # 該当の予算ID, 工事IDで作業中FLがONのレコード数をカウント
            on_progress_budget_num = ProInspectionResults.objects.filter(budget_id=budget_id, construction_id=construction_id, entry_on_progress_flag=1, lost_flag=0).count()
            # 該当の予算ID, 工事IDで(入力)完了FLがONのレコード数をカウント
            complete_entry_budget_num = ProInspectionResults.objects.filter(budget_id=budget_id, construction_id=construction_id, entry_on_progress_flag=0, lost_flag=0).count()
            # 完了FLがONの件数が「0」より多い場合
            if complete_entry_budget_num > 0:
                # 該当の予算idで、作業中FLがONのレコードを抽出し、主キーのidが最新のレコードを抽出
                proinspectioneesults_data = ProInspectionResults.objects.filter(budget_id=budget_id, construction_id=construction_id, entry_on_progress_flag=0, lost_flag=0).order_by('-id')[0]
                # 該当の予算idで最終のrev_noを取得
                latest_rev_no = proinspectioneesults_data.rev_no
                # 該当のレコードを無効
                proinspectioneesults_data.lost_flag = 1
                # 予算のレコードを保存
                proinspectioneesults_data.save()
            # 完了FLがONの件数が「0」の場合
            else:
            ...
            if on_progress_budget_num == 0:
                ...
                ProInspectionResults(budget_id=budget_id, construction_id=construction_id, entry_datetime=now, entry_operator=operator, lost_flag=0).save()
                # 登録日時、登録者で予算レコードを抽出
                proinspectioneesults_data = ProInspectionResults.objects.get(entry_datetime=now, entry_operator=operator, lost_flag=0)
                # 主キーを取得
                proinspectioneesults_unique_id = proinspectioneesults_data.id
                ...
                proinspectioneesults_data = ProInspectionResults.objects.get(id=proinspectioneesults_unique_id, lost_flag=0)
                # rev_no、作業中FL、無効FLに値を代入
                proinspectioneesults_data.rev_no = latest_rev_no + 1
                proinspectioneesults_data.entry_on_progress_flag = 1
                proinspectioneesults_data.lost_flag = 0
                # 予算のレコードを保存
                proinspectioneesults_data.save()
            # 該当の予算idで作業中FLがONのレコード数が「0」でない場合
                ...
                proinspectioneesults_data = ProInspectionResults.objects.get(budget_id=budget_id, construction_id=construction_id, entry_on_progress_flag=1, lost_flag=0)
                # 主キーを取得
                proinspectioneesults_unique_id = proinspectioneesults_data.id
        ...
        # 各項目の値を設定
        proinspectioneesults_data.sub_id = sub_id if sub_id is not '' else None
        if order_classification_from_date != "":
            proinspectioneesults_data.order_classification_from = order_classification_from_date
        if order_classification_to_date != "":
            proinspectioneesults_data.order_classification_to = order_classification_to_date
        proinspectioneesults_data.inspection_results = request.POST["inspection_results"] if request.POST["inspection_results"] is not '' else None
        proinspectioneesults_data.judge_kbn = request.POST["judge_kbn"] if request.POST["judge_kbn"] is not '' else None


        # 今のstepと次のstepが違う場合の処理
        if this_step != next_step:
            # 作業中FL=0　にする
            proinspectioneesults_data.entry_on_progress_flag = 0
        # 今のstepと次のstepが同じ場合の処理
        else:
            # 作業中FL=1　にする
            proinspectioneesults_data.entry_on_progress_flag = 1
        ...
        # rev_no取得
        proinspectioneesults_rev_no = proinspectioneesults_data.rev_no

        # 工事納入情報テーブルのレコードを保存<input type="hidden" id="permit_user" value="y-kawauchi" name="permit_user" size="3" style="width:150px; height:20px">
        proinspectioneesults_data.save()

        # ログデータを新規登録
        #Log(target='proinspectioneesults', target_id=construction_id, action=action, operator=operator, operation_datetime=now, step=this_step, comment=comment, operator_department=this_department, operator_division=this_division, budget_id=this_budget_id).save()
