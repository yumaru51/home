...
function __select__brand(value){
    $.ajax({
        url: "/isk_tools/report_output/" + group + "/品質検討会資料/__select__brand/",
        type: "POST",
        data : {
            'value': value,
...
function trace_report_download(brand, run_no1, run_no2, run_no3){
    ...
    var link_str = "/isk_tools/report_output/" + group + "/品質検討会資料/download/" + brand + "/" + run_no1 + "/" + run_no2 + "/" + run_no3 + "/";
...
function trace_no_numbering(){
    ...
    $.ajax({
        url: "/isk_tools/report_output/品質検討会資料/トレースNO採番/",
        type: "POST",
        data : {
